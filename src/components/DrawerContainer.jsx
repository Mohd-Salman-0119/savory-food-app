...
const DrawerContainer = () => {
    ...
    return (
        <div className="md:flex md:flex-row sticky top-0 left-0 h-auto w-full">
            {/* Small screen bar */}
            <div className="flex justify-between p-2 items-center md:hidden z-0 sticky top-0 left-0 right-0 w-full">
                <div>
                    <img src={logoWithoutText} className="w-10" />
                </div>
                <div className="flex gap-2 ">
                    <IconButton icon={faMagnifyingGlass} className="px-2 py-1 rounded-full bg-white w-max" />
                    <IconButton icon={faBell} className="px-2 py-1 rounded-full bg-white w-max" />
                    <IconButton
                        icon={faBars}
            ...
            {isVisibleOnSmall && <DrawerComponents isOpen={isOpen} onHandleBurgur={setIsOpen} />}
        </div>
    );
};
